...
public class Region extends StreamedWrapper {
	...
	public int[][][][] extraData = new int[4][width][height][5];
		...
		for (int plane = 0; plane < 4; plane++) {
			for (int x = 0; x < width; x++) {
				for (int y = 0; y < height; y++) {
					int operation = extraData[plane][x][y][4] = stream.getUByte();
					if ((operation & 0x1) != 0) {
						extraData[plane][x][y][0] = stream.getUByte();
						extraData[plane][x][y][1] = stream.getSmart();
					}
					if ((operation & 0x2) != 0)
						landscapeData[plane][x][y] = stream.getByte();
					if ((operation & 0x4) != 0)
						extraData[plane][x][y][2] = stream.getSmart();
					if ((operation & 0x8) != 0)
						extraData[plane][x][y][3] = stream.getUByte();
				}
			}
		}
		for (int plane = 0; plane < 4; plane++) {
